...
- OpenAI 格式：`POST /v1/videos`、`GET /v1/videos/{id}`、`GET /v1/videos/{id}/content`，图生视频以 `input_reference[]` 上传多张参考图。
- NewAPI 格式：仅在视频接口上与 OpenAI 不同，创建任务为 `POST /v1/video/generations`（文生视频传 `prompt`，图生视频以 `images` 数组传参考图，可用 `duration`/`width`/`height` 控制输出），查询任务为 `GET /v1/video/generations/{task_id}`，成功后响应直接返回 `url` 视频地址，无需单独下载接口。其余生图、文本、音频、模型列表等能力与 OpenAI 格式完全一致。
- 百炼格式（阿里云 DashScope）：默认 Base URL `https://dashscope.aliyuncs.com`，鉴权 `Authorization: Bearer`。生图为同步接口 `POST /api/v1/services/aigc/multimodal-generation/generation`，文生图传 `input.messages.content.text`、图生图/编辑在 content 里加 1-3 个 `image` 项（支持 base64 data URL），响应从 `output.choices[].message.content[].image` 取图片 URL。生视频为异步接口 `POST /api/v1/services/aigc/video-generation/video-synthesis`（带 `X-DashScope-Async: enable`），创建返回 `output.task_id`，轮询 `GET /api/v1/tasks/{task_id}`，`output.task_status` 为 `SUCCEEDED` 时取 `output.video_url`。百炼视频按模型族发不同参数：kling 系列发 `mode`(pro/std)/`aspect_ratio`/`duration`/`watermark`，wan 系列发 `resolution`/`duration`/`prompt_extend`/`watermark`。图生视频的 `input.media` 按参考图模式组合：首帧 `first_frame`、首尾帧 `first_frame+last_frame`、参考生视频 `refer`（仅 `kling/kling-v3-omni-video-generation` 支持多图）；模式可在视频设置面板选"自动/首帧/首尾帧/参考生视频"，自动=按图片数量（1→首帧、2→首尾帧、3+→参考生视频）。百炼格式暂不支持文本问答、音频生成和拉取模型列表。百炼视频轮询的 `/api/v1/tasks/` 端点不返回 CORS 头，浏览器直连会被跨域拦截，需把渠道 Base URL 指向反代 dashscope 的地址（开发态用 `web/vite.config.ts` 的 `/bailian` 代理，生产 Docker 部署用 `nginx.conf` 内置的 `/bailian/` 反代 location）。
